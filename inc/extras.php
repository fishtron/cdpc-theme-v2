...
if ( ! function_exists ( 'understrap_post_nav' ) ) {
	function understrap_post_nav() {
		...
		?>
		<nav class="container navigation post-navigation text-center">
			<h2 class="sr-only"><?php esc_html_e( 'Post navigation', 'understrap' ); ?></h2>
			<div class="row nav-links justify-content-between">
				<?php
				if ( get_previous_post_link() ) {
					previous_post_link( '<div class="nav-previous col-md-6 col-sm-12">%link</div>', _x( '<i class="fa fa-angle-left"></i>&nbsp;<em>Previous Post</em><br /> %title', 'Previous post link', 'understrap' ) );
				}
				if ( get_next_post_link() ) {
					next_post_link( '<div class="nav-next col-md-6 col-sm-12">%link</div>', _x( '<em>Next Post</em>&nbsp;<i class="fa fa-angle-right"></i><br />%title', 'Next post link', 'understrap' ) );
				}
				?>
			</div><!-- .nav-links -->
		</nav><!-- .navigation -->
